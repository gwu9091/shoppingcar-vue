<script>
import quantityinput from '@/views/quantityinput.vue'
...
export default {
    ...
    data() {
        return {
            quantities: [
                { quantity: 0, price: 100, name: '狗糧' },
                { quantity: 0, price: 150, name: '貓糧' },
                { quantity: 0, price: 50,  name: '魚飼料' },
            ],
    ...
    watch: {
        quantities: {
            handler(newQuantities) {
                // 1. 計算商品總數量 (totals.quantities)
                const totalQuantity = newQuantities.reduce((sum, item) => {
                    // 確保 quantity 是數字，並累加
                    return sum + Number(item.quantity);
                }, 0);

                // 2. 計算商品總小計 (totals.total1)
                const subtotal = newQuantities.reduce((sum, item) => {
                    // 數量 * 價格
                    return sum + (Number(item.quantity) * Number(item.price));
                }, 0);

                // 3. 更新 totals
                this.totals.quantities = totalQuantity;
                this.totals.total1 = subtotal;
                //運費固定 60
                this.totals.total2 = subtotal + 60;
            },
            deep: true,
            //添加 immediate: true，這樣組件加載時就會執行一次計算
            immediate: true
        }
    }
}
        ...
        <div class="container-lg  bg-white mywidth" style="height:95%;">
                ...
                <div>
                    <div class="d-flex">
                        ...
                        <div class="d-flex justify-content-center align-items-center ms-auto px-3">
                            <quantityinput v-model="quantities[0].quantity"></quantityinput>
                            <p class="m-0">$100</p>
                        </div>
                    </div>
                    <hr>
                    <div class="d-flex">
                        ...
                        <div class="d-flex justify-content-center align-items-center ms-auto px-3">
                            <quantityinput v-model="quantities[1].quantity"></quantityinput>
                            <p class="m-0">$150</p>
                        </div>
                    </div>
                    <hr>
                    <div class="d-flex">
                        ...
                        <div class="d-flex justify-content-center align-items-center ms-auto px-3">
                            <quantityinput v-model="quantities[2].quantity"></quantityinput>
                            <p class="m-0">$50</p>
                        </div>
                    </div>
                    <hr>
            ...
            <section class=" d-flex justify-content-between align-items-center px-4 pt-2">
                <p>
                    ...
                    <RouterLink class="link-offset-2 link-underline link-underline-opacity-0 link-dark" to="/">
                        返回購物
                    </RouterLink>
                </p>
                <RouterLink class="btn btn-primary" to="/shoppcar2" role="button" @click="save()">
                    下一步
                </RouterLink>
            </section>
        </div>
